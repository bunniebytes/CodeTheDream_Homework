...
class TicTacToe:
    ...
    def get_move(self):
        user_move = input(f"Please make a move {self.board.turn} (upper left, upper center, upper right, middle left, center, middle right, lower left, lower center, lower right): ")
        try:
            self.board.move(user_move)
            print(self.board)
            self.board.whats_next()
        except TictactoeException as e:
            print(e)
    
    def play_game(self):
        while not self.board.whats_next()[0]:
            self.get_move()
        print(f"{self.board.whats_next()[1]}")
